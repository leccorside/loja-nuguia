  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-sm font-black text-gray-800 uppercase tracking-widest">Variations</h1>
      </div>
        ...
        <div className="flex-1 space-y-6 w-full">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm">
                  ...
                  {filteredVariations.map((v, i) => (
                    <tr key={v.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-4 py-4 text-center relative action-menu-container">
                        ...
                        {openMenuId === v.id && (
                          <div className="absolute right-full top-1/2 -translate-y-1/2 mr-3 w-40 bg-white rounded-md shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] z-10 py-1.5 animate-in slide-in-from-right-2 duration-200 border border-gray-50/50">
                            <button 
                            ...
                            <Link 
                              ...
                              className="w-full flex items-center gap-3 px-4 py-2 text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors group"
                            >
                              <ListBulletIcon className="h-4 w-4 text-slate-400 group-hover:text-slate-600" />
                              Add Values
                            </Link>
            ...
            <div className="mt-8 pt-4 border-t border-gray-50">
               <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Showing 1-{filteredVariations.length} of {variations.length} results</p>
            </div>
          </div>

          {/* Add New Variation Card */}
          <div id="add-variation-form" className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-gray-50 pb-4">
              <h2 className="text-[13px] font-black text-gray-800 uppercase tracking-widest">{editingId ? "Update Variation" : "Add New Variation"}</h2>
              {editingId && (
                <button 
                  onClick={cancelEdit}
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-black text-gray-700 uppercase tracking-wider">Variation Name</label>
              <input 
                type="text" 
