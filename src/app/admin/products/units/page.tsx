  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm flex items-center justify-between">
        <h1 className="text-sm font-black text-gray-800 uppercase tracking-widest">Units</h1>
      </div>
        ...
        <div className="flex-1 space-y-6 w-full">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm">
                  ...
                  {filteredUnits.map((u, i) => (
                    <tr key={u.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <span className="text-[11px] font-black text-gray-800">{u.name}</span>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <button 
                          onClick={() => toggleStatus(u.id)}
                          className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors duration-200 focus:outline-none ${u.active ? 'bg-green-500' : 'bg-gray-200'}`}
                        >
            ...
            <div className="mt-8 pt-4 border-t border-gray-50">
               <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Showing 1-{filteredUnits.length} of {units.length} results</p>
            </div>
          </div>

          {/* Add Form Card */}
          <div id="add-unit-form" className="bg-white p-4 sm:p-6 rounded-sm border border-gray-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-gray-50 pb-4">
              <h2 className="text-[13px] font-black text-gray-800 uppercase tracking-widest">{editingId ? "Update Unit" : "Add New Unit"}</h2>
              {editingId && (
                <button 
                  onClick={cancelEdit}
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-black text-gray-700 uppercase tracking-wider">Unit Name</label>
              <input 
                type="text" 
