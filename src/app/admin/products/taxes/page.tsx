  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-sm border border-gray-100 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
        <h1 className="text-sm font-black text-gray-800 dark:text-gray-100 uppercase tracking-widest transition-colors">Taxes</h1>
      </div>
        ...
        <div className="flex-1 space-y-6 w-full">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-sm border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                  {filteredTaxes.map((t, i) => (
                    <tr key={t.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800 transition-colors group">
                      ...
                      <td className="px-4 py-4 transition-colors">
                        <span className="text-[11px] font-black text-gray-800 dark:text-gray-100 uppercase transition-colors">{t.name}</span>
                      </td>
                      <td className="px-4 py-4 text-center transition-colors">
                        <button 
                          onClick={() => toggleStatus(t.id)}
                          className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors duration-200 focus:outline-none ${t.active ? 'bg-green-500 shadow-sm shadow-green-100 dark:shadow-none' : 'bg-gray-200 dark:bg-slate-700'}`}
                        >
            ...
            <div className="mt-8 pt-4 border-t border-gray-50 dark:border-slate-800 transition-colors">
               <p className="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest transition-colors">Showing 1-{filteredTaxes.length} of {taxes.length} results</p>
            </div>
          </div>

          {/* Add Form Card */}
          <div id="add-tax-form" className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-sm border border-gray-100 dark:border-slate-800 shadow-sm space-y-6 transition-colors">
            <div className="flex items-center justify-between border-b border-gray-50 dark:border-slate-800 pb-4 transition-colors">
              <h2 className="text-[13px] font-black text-gray-800 dark:text-gray-100 uppercase tracking-widest transition-colors">{editingId ? "Update Taxes" : "Add New Taxes"}</h2>
              {editingId && (
                <button 
                  onClick={cancelEdit}
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-black text-gray-700 dark:text-slate-300 uppercase tracking-wider transition-colors">Tax Name</label>
              <input 
                type="text" 
                placeholder="Type tax name"
                value={taxName}
                onChange={(e) => setTaxName(e.target.value)}
                className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-sm text-xs font-medium focus:ring-1 focus:ring-green-500 outline-none transition-all placeholder:text-gray-300 dark:placeholder:text-slate-600 font-bold text-gray-800 dark:text-gray-100"
              />
            </div>
